...
for t=0,timesteps do begin
   ...
   for i=0,n-1 do begin
      r_sqr = (x(i)-xc)^2
      q(i) = e0 / ((4.0*!pi*D*time)^(0.5)) * exp(-r_sqr/(4.0*D*time))
   endfor
   ...
   ;i_up(0) = i_up_old(0);q(0);s2*(1.0-exp(-dtau));qdr(0.0,s2,s3,dtau,dtau,3);s(0);kappa/kappa_hat*(s(0)+1.0/(kappa)*dIdt_fact*i_up_old(0))
   i_up(0) = qdr(0.0,s2,s3,dtau,dtau,1);s(0);kappa/kappa_hat*(s(0)+1.0/(kappa)*dIdt_fact*i_up_old(0))
   ...
   ;i_down(n-1) = i_down_old(n-1);q(n-1);s2*(1.0-exp(-dtau));qdr(0.0,s2,s3,dtau,dtau,3);s(0);kappa/kappa_hat*(s(0)+1.0/(kappa)*dIdt_fact*i_up_old(0))
   i_down(n-1) = qdr(0.0,s2,s3,dtau,dtau,1);s(0);kappa/kappa_hat*(s(0)+1.0/(kappa)*dIdt_fact*i_up_old(0))
   ...
   if(dt lt dt_init) then dt = dt_init
...
if(wf) then begin
   nx = string(n,FORMAT='(i3.3)')
   outfile = 'diffusion_1D_' + nx + '.dat'
   openw,1, outfile
...
print, "evoled time/initial time:",(time-t0)/t0
