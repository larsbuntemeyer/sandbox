...
for i=0,n-1 do begin
   ...
   s(i) = q0(i)
   j(i) = 0.0 
   i_up_old(i)   = q0(i) ;e0 / ((4.0*!pi*D*(t0-dt))^(0.5)) * exp(-r_sqr/(4.0*D*(t0-dt)))
   i_down_old(i) = q0(i) ;i_up_old(i) 
endfor
...
;plot time-independent solution
;oplot, x, j0, psym=-2
...
for t=0,timesteps do begin
   ...
   i_up(0) = q(0);s1*exp(-dtau);qdr(s1,s2,s3,dtau,dtau,1)
   for i=1,n-2 do begin
   ...
   i_down(n-1) = q(n-1);s1*exp(-dtau) ;qdr(s1,s2,s3,dtau,dtau,1)
   for i=n-2,1,-1 do begin
