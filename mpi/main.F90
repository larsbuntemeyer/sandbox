...
!
   ...
   allocate(sendArray(sizeOfMessage,1))
   allocate(receiveArray(sizeOfMessage,nrOfProcs))
...
!
   ...
   do i=1,sizeOfMessage
      sendArray(i,1) = dble(myRank*sizeOfMessage+i)
      receiveArray(i,myRank) = sendArray(i,1)
      if(wf) write(io,*) i,sendArray(i,1)
   enddo
!
...
!
   receiveArray = 0.d0
   do i=1,sizeOfMessage
      sendArray(i,1) = (myRank*sizeOfMessage+i)
      receiveArray(i,myRank) = sendArray(i,1)
   enddo
!
   call MPI_BARRIER(MPI_COMM_WORLD,ierr)
...
!
!
   call MPI_ALLGATHER(sendArray,sizeOfMessage,MPI_DOUBLE_PRECISION,      &
...
!
   if(check) then
      do j=0,nrOfProcs-1
         ...
         do i=1,sizeOfMessage
            l = j*sizeOfMessage+i
            if(receiveArray(i,j).ne.dble(l)) then
              write(io,*) 'ERROR in Communication with MPI_ALLGATHER'
              write(*,*) 'ERROR in Communication with MPI_ALLGATHER'
              write(*,*) 'i',i
              write(*,*) 'j',j
              write(*,*) 'l',l
              write(*,*) 'receiveArray(i,j)',receiveArray(i,j)
              stop
            endif
            if(wf) write(io,*) receiveArray(i,j)
         enddo
      enddo
   endif
!
